...
def Euclidean_norm(fm: Tensor, compress) -> Tensor:
    ...
    fm1_norm = (fm1-fm1_min_vals) / (fm1_max_vals - fm1_min_vals + 1e-10)

    fm2_max_vals = torch.amax(fm2, axis=[2,3], keepdim=True)
    fm2_min_vals = torch.amin(fm2, axis=[2,3], keepdim=True)
    fm2_norm = (fm2-fm2_min_vals) / (fm2_max_vals - fm2_min_vals + 1e-10)

    diff = compress(fm1_norm) - compress(fm2_norm)

    L2_norm = torch.sqrt( torch.mean( torch.pow(diff + 1e-10, 2) , axis=(2, 3)) )
    return L2_norm
